...
{:then api}
	<header class="mb-1 mt-5">
		...
		<p class="text-md font-normal text-gray-500 lg:text-lg dark:text-gray-400">
			<a href={api.data.monitors[0].url} title={`Go to ${api.data.monitors[0].friendly_name}`} rel="noopener" target="_blank"><mark
			class="px-1 font-bold bg-neutral-900 text-neutral-100 dark:bg-neutral-100 dark:text-neutral-900 rounded-md"
			><b>{api.data.monitors[0].friendly_name}</b></mark></a> was currently in the <mark
			class="px-1 font-bold bg-neutral-900 text-{api.data.monitors[0].status} dark:bg-neutral-100 dark:text-{api.data.monitors[0].status} rounded-md"
	...
	<main>
		...
		{#each api.data.monitors[0].logs as logs}                  
			<li class="mb-5 ms-6">            
				<span class="absolute flex items-center justify-center w-6 h-6 rounded-full -start-3 ring-8 dark:ring-neutral-400 ring-neutral-700 bg-neutral-900 dark:bg-neutral-100">
					<svg class="w-2.5 h-2.5 text-neutral-200 dark:text-neutral-900" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
						<path d="M20 4a2 2 0 0 0-2-2h-2V1a1 1 0 0 0-2 0v1h-3V1a1 1 0 0 0-2 0v1H6V1a1 1 0 0 0-2 0v1H2a2 2 0 0 0-2 2v2h20V4ZM0 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8H0v10Zm5-8h10a1 1 0 0 1 0 2H5a1 1 0 0 1 0-2Z"/>
					</svg>
				</span>
				<h3 class="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">#{logs.id} <span class="bg-neutral-900 dark:text-neutral-900 text-neutral-100 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-neutral-100 ms-3">
					Code°{logs.reason.code}
				</span></h3>
				<time class="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Created on <Time relative timestamp={new Date(logs.datetime * 1000)} /></time>
				<p class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">Details: {logs.reason.detail} <br> Duration: {formatDuration(logs.duration)} </p>
			</li>
		{/each}
		</ol>
	</main>
